...
export default function BrochureModal({ isOpen, onClose }) {
  ...
  const handleSubmit = (e) => {
    ...
    const whatsappMessage = `📋 *Brochure Request - AWS DevOps Training Hyderabad*

👤 *Name:* ${formData.name}
📧 *Email:* ${formData.email}
📱 *Phone:* ${formData.phone}
💼 *Interest:* ${formData.interest || 'All Courses'}

🏢 *Institute:* awsdevopstraininghyderabad.com
📍 *Location:* HITEC City, Hyderabad

Hi! I would like to receive the detailed course brochure with curriculum, fees, and batch timings. Please share the complete information.
...
Thank you!`;
    ...
    alert(`Thank you ${formData.name}! Redirecting to WhatsApp to get your brochure and detailed information.`);
    onClose();
    setFormData({
      name: '',
      email: '',
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
      <div className="relative max-w-md w-full mx-4 bg-slate-800 rounded-2xl border border-slate-600 p-8">
        ...
        <div className="text-center mb-6">
          ...
          <h2 className="text-2xl font-bold text-white mb-2">Download Brochure</h2>
          <p className="text-gray-400 text-sm">Get detailed information about our courses, curriculum, and pricing</p>
        </div>
        ...
        <div className="mt-4 text-xs text-gray-500 text-center">
          By downloading, you agree to receive communications from VR-IT-AWS
        </div>
      </div>
    </div>
  );
